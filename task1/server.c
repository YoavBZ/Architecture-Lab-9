...
int handle_client(int client_fd){
    ...
    while (1){
        ...
        if (0 == strncmp(buffer, "hello", 5)){
            ...
            free_cmd_lines(cmd);
            return 0;
        } else if (0 == strncmp(cmd->arguments[0], "ls", 2)){
            if (CONNECTED != state.conn_state){
                nok(client_fd, "state");
                return -2;
            }

            assert(send(client_fd, "ok", sizeof("ok"), 0), "Error in send()");
            char *files = list_dir();
            if (NULL == files){
                nok(client_fd, "filesystem");
                return -2;
            }
            if (0 > send(client_fd, files, strlen(files), 0)){
                free(files);
...
int main(int argc, char **argv){
    ...
    // state.server_addr = gethostbyname(gethostname(hostname, NAME_SIZE))->h_name;
